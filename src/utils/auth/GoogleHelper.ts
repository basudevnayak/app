...
export const handleGoogleSignInNative = async (
  ...
  try {
    ...
    if (response?.type === "success") {
      ...
      await AsyncStorageHelper.setItem("user", JSON.stringify(firebaseResult.user));
...
// Web sign-in handler
export const handleGoogleSignInWeb = async (navigation: any) => {
  try {
    Logger.log("🌐 Web Google Sign-In initiated");

    const result = await signInWithPopup(auth, googleProvider);
    await AsyncStorageHelper.setItem("user", JSON.stringify(result.user));
