...
const EditProduct: React.FC = () => {
  ...
  const { product } = route.params as { product: { name: string; deviceModel: string; company: string; description: string; image?: string; uuid: string } };
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.put(`${BASE_URL}/product/${product.uuid}`, data, {
        headers: {
  ...
  return (
    <View style={styles.mainContainer}>
      <CommonHeader title="Edit Product" onBackPress={() => navigation.goBack()} />
